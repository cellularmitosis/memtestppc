 ...
 *
 * ----------------------------------------------------
 * memtestppc+ port: line-faithful port of memtest86+ v2.00 screen_buffer.c to
 * PowerPC / Open Firmware. This is platform-neutral plumbing: it keeps a
 * char-only shadow screen_buf[][] and, in tty_print_line()/tty_print_region(),
 * compares against it to find the changed run, then calls ttyprint(y,x,text) to
 * emit only the changed cells. NOTHING here is x86-specific, so the file is
 * imported VERBATIM (zero code changes). The display leaf ttyprint() is
 * declared in test.h and defined in lib.c — there it renders to the
 * framebuffer instead of a serial console. SCRN_DEBUG is left OFF exactly as
 * upstream ships it. See docs/sessions/006-port-fidelity-restructure/
 * PORTING-GUIDE.md.
 */
#include "test.h"
